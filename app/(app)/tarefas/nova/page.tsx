...
export default async function NewTaskPage({ searchParams }: { searchParams: Promise<{ projeto?: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl animate-fade-in"><PageHeader title="Nova tarefa" description="Defina responsáveis, prazo, prioridade e participantes." /><TaskForm action={createTaskAction} projects={projects} users={availableUsers} tasks={tasks.map((task) => ({ id: task.id, name: task.title }))} defaults={{title:"",description:"",projectId:projects.some((p)=>p.id===projeto)?projeto:"",responsibleId:"",participantIds:[],priority:"MEDIUM",status:"NOT_STARTED",startDate:"",dueDate:"",estimatedHours:"",tagNames:"",dependsOnId:"",blockReason:""}} /></div>;
}
